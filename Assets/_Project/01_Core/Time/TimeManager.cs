...
namespace TSI.Core.Time
{
    public class TimeManager : ITickable
    {
        ...
        private readonly Dictionary<TimeLayer, IClock> _clocks;
        ...
        public T GetClock<T>(TimeLayer layer) where T : class, IClock
        {
        ...
        public IClock GetClock(TimeLayer layer)
        {
            return _clocks.GetValueOrDefault(layer);
        }
        
        public IFixedClock GetFixedClock(TimeLayer layer)
        {
            var clock = _clocks.GetValueOrDefault(layer);
